...
#ifdef OPUS_X64
#define offset(s, field)      ((size_t)offsetof(struct s, field))
#else
...
#ifdef OPUS_X64
/* The separate localization-key pass is not present in this source drop.
 * Keep the original English resource text and do not expand the absent key
 * token into a C identifier. */
#define StFrameKey(s, key) StringMap(s, 1, 1)
#define SzFrameKey(s, key) s
#define StSharedKey(s, key) StringMap(s, 1, 2)
#define SzSharedKey(s, key) s
#else
...
#ifdef OPUS_X64
#define StGlobalKey(s, key) StringMap(s, 1, 0)
#define SzGlobalKey(s, key) s
#else
...
#ifdef OPUS_X64
static __inline void OpusMovePwToPch(const void *pw, char *pch)
{
	const unsigned char *pb = (const unsigned char *)pw;
	pch[0] = (char)pb[0];
	pch[1] = (char)pb[1];
}
#define MovePwToPch(pw, pch) OpusMovePwToPch((pw), (char *)(pch))
#elif defined(WIN)
...
#ifdef OPUS_X64
/* A 16-bit near handle fit in the compiler's implicit int return type.  On
 * Win64 these constructors must be declared before every K&R-era call site
 * or the upper half of the movable-handle pointer is discarded. */
struct STTB **HsttbInit();
struct STTB **HsttbInit1();
#endif
...
#ifdef OPUS_X64
NATIVE struct PLC **HplcCreateEdc();
#endif
NATIVE struct PL **HplInit();
NATIVE struct PL **HplInit2();
#ifdef OPUS_X64
NATIVE uns **HCopyHeapBlock();
#endif
...
NATIVE struct STTB **HsttbAssocEnsure();
